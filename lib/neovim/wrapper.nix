...
let
  wrapper =
    ...
    let
      rc = if lua then "init.lua" else "init.vim";
      manifest = if lua then "manifest.lua" else "manifest.vim";
      ...
      finalMakeWrapperArgs =
        ...
        ++ optionals wrapRc [ "--add-flags" "-u ${writeText rc neovimRcContent}" ]
      ;
    in
      ...
      + optionalString (manifestRc != null) (
        ...
        ''
          ...
          if ! $out/bin/nvim-wrapper \
            -u ${writeText manifest manifestRc} \
            -i NONE -n \
            -E -V1rplugins.log -s \
